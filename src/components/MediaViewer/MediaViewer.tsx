...
const MediaViewer = ({
    ...
    const validInitialIndex = Math.max(0, Math.min(initialIndex, attachments.length - 1));
    ...
    useEffect(() => {
        if (isOpen && attachments.length > 0) {
            const newIndex = Math.max(0, Math.min(initialIndex, attachments.length - 1));
    ...
    const handleWheel = (e: WheelEvent): void => {
        ...
        const isTouchpad = !isPinch && e.deltaMode === 0 && (Math.abs(e.deltaY) < 50 || Math.abs(e.deltaX) < 50);
    ...
    const handleGestureChange = (e: any): void => {
        e.preventDefault();
        const scaleFactor = e.scale > 1 ? 1 + (e.scale - 1) * 0.5 : 1 / (1 + (1 - e.scale) * 0.5);
    ...
    useEffect(() => {
        ...
        movers.addEventListener("gesturestart", handleGestureStart, { passive: false });
        movers.addEventListener("gesturechange", handleGestureChange, { passive: false });
        return () => {
    ...
    const handleTouchMove = (e: TouchEvent): void => {
        if (e.touches.length === 2) {
            ...
            const touch2 = e.touches[1];
            const distance = Math.hypot(touch2.pageX - touch1.pageX, touch2.pageY - touch1.pageY);
            const initialDistance = parseFloat((e.currentTarget as HTMLElement).dataset.initialDistance ?? "1");
    ...
    const handleMouseMove = (e: MouseEvent): void => {
        if (isDragging && dragStartRef.current && mediaRef.current && viewerRef.current) {
            e.preventDefault();
            const { maxX, maxY } = calculateBounds();
            if (!maxX && !maxY) return;
    ...
    if (!isOpen || !attachments.length || currentIndex < 0 || currentIndex >= attachments.length) {
        return null;
    }
    ...
    const handleDownload = async (): Promise<void> => {
        try {
            ...
            const filename = currentAttachment.filename || `attachment_${currentAttachment.uuid}.${extension}`;
            ...
            const link = document.createElement("a");
            link.href = url;
            link.download = currentAttachment.filename || `attachment_${currentAttachment.uuid}`;
    ...
    const viewerContent = (
        ...
        >
            ...
            <div className={styles.mediaViewerContent}>
                <div className={`${styles.mediaViewerTopbar} ${styles.mediaViewerAppear}`}>
                    <div className={styles.mediaViewerTopbarLeft}>
                        <button className={`${styles.btnIcon} ${styles.onlyHandhelds}`} onClick={handleClose}>
                    ...
                    <div className={styles.mediaViewerButtons}>
                        ...
                        <button className={styles.btnIcon} onClick={() => { handleZoom(scale + zoomStep); }}>
                ...
                <div className={`${styles.zoomContainer} ${isZooming ? styles.isVisible : ""}`}>
                    <button className={styles.btnIcon} onClick={() => { handleZoom(scale - zoomStep); }}>
                        <img src={ZoomOutIcon} alt="Zoom Out" />
                    </button>
                ...
                {hasCaption && (
                    <div className={`${styles.mediaViewerCaption} ${isZooming ? styles.hide : ""}`}>
                        <div className={styles.scrollableY}>{currentAttachment.filename}</div>
                    </div>
                )}
            </div>
        </div>
